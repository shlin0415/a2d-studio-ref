"""
Test: Dual Port Performance (No Model Switching)
=================================================

Tests the time cost when using two GPT-SoVITS ports (31801, 31802) without model switching.

Scenario:
- 2 characters (Ema, Hiro) with different models
- Each character has its own dedicated port with pre-loaded models
- 10 dialogue turns (alternating characters)
- No model loading needed during dialogue

Usage:
    python test_dual_port_performance.py

Requirements:
    - GPT-SoVITS API running at http://127.0.0.1:31801 (Ema)
    - GPT-SoVITS API running at http://127.0.0.1:31802 (Hiro)
...
async def run_dual_port_test(
    ...
    """Run test using dual ports without model switching"""

    # Create clients for each port
    ema_client = GPTSoVITSClient(api_url=f"http://127.0.0.1:{ema_port}")
    hiro_client = GPTSoVITSClient(api_url=f"http://127.0.0.1:{hiro_port}")

    print("\n" + "=" * 70)
    print("DUAL PORT TEST (No Model Switching)")
    print("=" * 70)
    print(f"Ema port: {ema_port}")
    print(f"Hiro port: {hiro_port}")
    print(f"Dialogue turns: {len(dialogue_lines)}")
    print("=" * 70)

    # Pre-load models on each port (one-time setup)
    print("\n--- Pre-loading models ---")

    print(f"\n[Ema port {ema_port}] Loading GPT: {ema_manager.gpt_model_path.name}")
    ema_gpt_time = await ema_client.set_gpt_weights(str(ema_manager.gpt_model_path))
    print(f"  Done in {ema_gpt_time:.2f}s")

    print(f"[Ema port {ema_port}] Loading SoVITS: {ema_manager.sovits_model_path.name}")
    ema_sovits_time = await ema_client.set_sovits_weights(
        str(ema_manager.sovits_model_path)
    )
    print(f"  Done in {ema_sovits_time:.2f}s")

    print(f"\n[Hiro port {hiro_port}] Loading GPT: {hiro_manager.gpt_model_path.name}")
    hiro_gpt_time = await hiro_client.set_gpt_weights(str(hiro_manager.gpt_model_path))
    print(f"  Done in {hiro_gpt_time:.2f}s")

    print(
        f"[Hiro port {hiro_port}] Loading SoVITS: {hiro_manager.sovits_model_path.name}"
    )
    hiro_sovits_time = await hiro_client.set_sovits_weights(
        str(hiro_manager.sovits_model_path)
    )
    print(f"  Done in {hiro_sovits_time:.2f}s")

    total_model_load_time = (
        ema_gpt_time + ema_sovits_time + hiro_gpt_time + hiro_sovits_time
    )
    print(f"\nTotal model load time (one-time): {total_model_load_time:.2f}s")
    ...
    for i, line in enumerate(dialogue_lines):
        ...
        # Determine which character and client
        if character == "艾玛":
            manager = ema_manager
            client = ema_client
            char_key = "ema"
        ...
        print(f"\n--- Turn {i + 1}: {char_key} (port {port}) ---")
        print(f"Text: {text[:40]}...")

        # Generate voice (no model loading needed)
    ...
    await ema_client.close()
    await hiro_client.close()

    # Calculate summary
    num_turns = len(dialogue_lines)
    summary = {
        "test_type": "dual_port_no_switching",
        "ema_port": ema_port,
        "hiro_port": hiro_port,
        "num_turns": num_turns,
        "one_time_model_load_time": total_model_load_time,
        "total_time": total_model_load_time + total_generation_time,
        "avg_generation_time": total_generation_time / num_turns,
        "avg_total_time": (total_model_load_time + total_generation_time) / num_turns,
...
async def main():
    ...
    hiro_client = GPTSoVITSClient(api_url=f"http://127.0.0.1:{hiro_port}")
    hiro_healthy = await hiro_client.health_check()
    await hiro_client.close()
    ...
    result = await run_dual_port_test(
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("DUAL PORT TEST RESULTS")
    print("=" * 70)
    print(f"Total turns: {result['summary']['num_turns']}")
    print(f"One-time model load: {result['summary']['one_time_model_load_time']:.2f}s")
    print(f"Total generation time: {result['summary']['total_generation_time']:.2f}s")
    print(f"Total time (including setup): {result['summary']['total_time']:.2f}s")
    print(
        f"Avg generation time per turn: {result['summary']['avg_generation_time']:.2f}s"
    )
